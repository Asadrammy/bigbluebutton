        ...
        try:
            ...
            if language:
                try:
                    ...
                    logger.warning(f"Unknown language: {language}, using English")
            
            logger.info(f"🔄 Transcribing real audio (language hint: {lang_enum})")
            
            # Transcribe with Whisper
            result = await self._transcribe_with_whisper(audio_bytes, lang_enum, return_timestamps=False)
            
            # Calculate processing time
            processing_time = time.time() - start_time
            result['processing_time'] = round(processing_time, 2)
            
            logger.info(f"✅ Real transcription complete: '{result.get('text', '')}' in {processing_time:.2f}s")
            return result
