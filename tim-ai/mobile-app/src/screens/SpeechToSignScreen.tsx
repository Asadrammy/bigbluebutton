...
import { mockSpeechToText, mockTextToSign } from '@services/mockData';
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          console.warn('Audio upload failed, continuing with processing:', uploadError);
        ...
        const sttResult = await mockSpeechToText();
        setSpokenText(sttResult.text);
        await mockTextToSign();
        setShowAvatar(true);
      }
    } catch (error) {
      Alert.alert(t.common.error, 'Failed to process audio');
    } finally {
  ...
  return (
    <View style={[styles.container, containerStyle]}>
      ...
      <View style={[styles.avatarContainer, containerStyle]}>
        {showAvatar && avatarAnimationData ? (
          ...
          <View style={[styles.avatarInactive, { backgroundColor: theme.colors.background }]}>
            <AvatarRenderer isLoading={false} />
          </View>
        )}
        ...
        {/* Results Display - Overlay on Avatar */}
        {spokenText && !isProcessing && (
          <View style={styles.resultsOverlay}>
            <View style={[styles.resultCard, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border }]}>
              <Text style={[styles.resultText, { color: theme.colors.text }]}>{spokenText}</Text>
            </View>
          </View>
        )}
