...
function getDevApiUrl(): string {
  ...
  try {
    ...
    // Warn if port is missing (defaults to 80 for HTTP)
    if (!url.port && url.protocol === 'http:') {
      console.error(
        '⚠️ API URL missing port! Backend runs on port 8000.\n' +
        `Current URL: ${baseUrl}\n` +
        `Expected format: http://YOUR_IP:8000/api/v1\n` +
        `Please update API_BASE_URL in .env file`
      );
    }
    
    // If using localhost and on a physical device, warn the developer
    if (url.hostname === 'localhost' && !Platform.isTV) {
      console.warn(
    ...
    console.error('⚠️ Invalid API_BASE_URL format:', baseUrl, e);
  }
