...
class AudioProcessor:
    ...
    def load_audio_from_bytes(audio_bytes: bytes, format: str = "wav") -> Tuple[np.ndarray, int]:
        ...
        Returns:
            Tuple of (audio_data, sample_rate)
        """
        # Try librosa first (better error handling, supports more formats)
        try:
            import librosa
            
            # Save to temporary file for librosa
            import tempfile
            suffix = f".{format}" if format != "m4a" else ".m4a"
            ...
            try:
                ...
                # Normalize
                audio_array = AudioProcessor.normalize_audio(audio_array)
                
                return audio_array, AudioProcessor.SAMPLE_RATE
            ...
            logger.debug("librosa not available, trying pydub...")
        except Exception as e:
            logger.warning(f"librosa failed: {e}, trying pydub...")
        ...
        try:
            ...
            error_msg = (
                ...
                "  After installation, add FFmpeg 'bin' folder to PATH and restart backend."
            )
            logger.error(error_msg)
            raise RuntimeError(error_msg)
        
        except ImportError:
            logger.error("pydub required for loading audio from bytes")
