...
export default function ForgotPasswordForm({ invalidToken }: PropsType) {
  ...
  async function onSubmit(data: Inputs) {
    ...
    try {
      ...
      const message = (error as { message?: string })?.message || 'Unable to send reset email.';
      toast.error(message);
    } finally {
      setIsLoading(false);
    }
    ...
    <>
      ...
      {isSent && (
        <div className="mb-6 rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-800 dark:border-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-200">
          Reset email sent. Follow the link in your inbox to finish resetting your password.
        </div>
      )}
