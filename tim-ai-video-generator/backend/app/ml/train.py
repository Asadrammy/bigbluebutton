...
from app.ml.config import ModelConfig, get_model_config, save_config
...
class TrainingOrchestrator:
    ...
    async def start_training(
        ...
        sign_language_code = dataset.sign_language
        
        # Create configuration
        model_dir = Path("models") / (sign_language_code or "generic")
        log_dir = Path("logs/training") / (sign_language_code or "generic")
    ...
    async def _run_training(
        ...
        try:
            ...
            # Create trainer
            # NOTE: In production, you'd create optimizer, scheduler, etc.
            ...
            trainer = Trainer(
                ...
                config=config,
                optimizer=None,  # Create optimizer here
                scheduler=None,  # Create scheduler here
            ...
            save_config(config, str(config.model_dir / "config.json"))
            ...
            await self._sync_sign_language_model(config.sign_language_code, model_version)
    ...
    async def _create_model_version(
        ...
        model_version = ModelVersion(
            ...
            sign_language_code=config.sign_language_code,
            is_active=False,
            is_deployed=False
        )
    ...
    async def _sync_sign_language_model(
        self,
        sign_language_code: Optional[str],
        model_version: ModelVersion,
    ) -> None:
        """Update or create the active model entry for a sign language."""
        if not sign_language_code:
            logger.info("No sign language code supplied; skipping registry sync")
            return
        result = await self.db.execute(
            select(SignLanguageEntity).where(SignLanguageEntity.code == sign_language_code)
        )
        sign_language = result.scalar_one_or_none()
        if not sign_language:
            logger.warning("Sign language %s not found in registry", sign_language_code)
            return

        result = await self.db.execute(
            select(SignLanguageModel).where(SignLanguageModel.sign_language_id == sign_language.id)
        )
        entry = result.scalar_one_or_none()

        if entry:
            entry.model_version_id = model_version.id
            entry.model_path = model_version.model_path
            entry.accuracy = model_version.val_accuracy or model_version.accuracy
            entry.is_active = True
        else:
            entry = SignLanguageModel(
                sign_language_id=sign_language.id,
                model_version_id=model_version.id,
                model_path=model_version.model_path,
                accuracy=model_version.val_accuracy or model_version.accuracy,
                is_active=True,
            )
            self.db.add(entry)
