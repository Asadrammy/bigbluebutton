...
class SignLanguageInference:
    ...
    def load_model(self, model_path: Optional[str] = None, sign_language: Optional[str] = None):
        ...
        """
        if not model_path:
            model_path = self.model_loader.resolve_model_path(sign_language)
        ...
        self.active_language = sign_language.upper() if isinstance(sign_language, str) else None
    ...
    def predict(
        ...
        """
        requested_language = sign_language.upper() if isinstance(sign_language, str) else None
        needs_reload = self.model is None or (
            requested_language and requested_language != self.active_language
        ) or (
            model_path and model_path != self.active_model_path
        )

        if needs_reload:
            self.load_model(model_path=model_path, sign_language=requested_language)
        ...
        try:
            ...
            language_suffix = f" [{requested_language}]" if requested_language else ""
            logger.info(
                f"Prediction{language_suffix}: {predictions['predicted_sign']} "
    ...
    def predict_batch(
        ...
        return_probabilities: bool = False,
        sign_languages: Optional[List[Optional[str]]] = None
    ) -> List[Dict]:
        """
        ...
        for i, video_frames in enumerate(video_frames_list):
            try:
                lang = None
                if sign_languages and i < len(sign_languages):
                    lang = sign_languages[i]
                pred = self.predict(video_frames, return_probabilities, lang)
    ...
    def predict_from_video_file(
        ...
        # Predict
        return self.predict(frames_array, return_probabilities, sign_language)
    ...
    def _run_inference(self, frames: np.ndarray) -> np.ndarray:
        ...
        Args:
            frames: Preprocessed frames (B, T, H, W, C)
        ...
        # with torch.no_grad():
        #     logits = self.model(torch.from_numpy(frames))
